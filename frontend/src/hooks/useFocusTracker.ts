...
import { FaceDetector, FaceLandmarker, HandLandmarker, ObjectDetector, PoseLandmarker, FilesetResolver } from '@mediapipe/tasks-vision'
import { focusConfig } from '@/config/focusConfig'
import { createPhoneEvidence, detectPhoneInHands } from './phoneDetection'
import type { PhoneEvidence } from './phoneDetection'
 ...
 *
 * Detection is multi-signal and smoothed rather than reading single raw
 * frames at face value:
 *  - Face presence/count is confidence-filtered, and a second face has
 *    to be sustained (not just a one-tick flicker) before MULTIPLE_FACES
 *    fires — a hand or phone raised near the face can momentarily read
 *    as a faint "second face" to the lightweight detector otherwise.
 *  - Hand-near-face (phone) and head-turn (yaw) are majority-vote
 *    smoothed over the last few ticks before their own sustained-timers
 *    even start, so a single noisy frame can't start or break either.
 *  - Drowsy is three independent signals, any one of which is enough:
 *    continuous eyes-closed, PERCLOS (the standard drowsiness-research
 *    metric — % of a rolling window with eyes closed, which catches
 *    heavy/frequent blinking that never holds shut long enough on its
 *    own), and a sustained yawn.
 *
...
interface Buckets {
  ...
  /**
   * A real "cell phone" object detection with a hand at/near it,
   * sustained — see ObjectDetector below. Falls back to the coarser
   * hand-near-face proxy only if that model fails to load.
   */
  phone: number
...
/**
 * Create a MediaPipe task on the GPU delegate (WebGL) when enabled, and
 * prove it works with one throwaway inference on a blank canvas — some
 * browsers/drivers create a GPU task fine and then throw on first use.
 * Any failure closes the half-built task and falls back to CPU. The
 * warm-up also compiles the GPU shaders up front, so the first real
 * frame doesn't stall.
 */
async function createWithGpuFallback<T extends { close(): void }>(
    create: (delegate: 'GPU' | 'CPU') => Promise<T>,
    warmUp: (task: T, blank: HTMLCanvasElement) => void,
): Promise<T> {
  if (focusConfig.PREFER_GPU_DELEGATE) {
    let gpuTask: T | null = null
    try {
      gpuTask = await create('GPU')
      const blank = document.createElement('canvas')
      blank.width = 64
      blank.height = 64
      warmUp(gpuTask, blank)
      return gpuTask
    } catch {
      try {
        gpuTask?.close()
...
/**
 * Majority-vote smoothing over the last few raw per-tick booleans. A
 * single noisy frame — a landmark jitter, a half-second partial
 * occlusion, one bad detection — no longer flips a signal on its own;
 * at least half of the recent samples have to agree. Applied to the
 * three noisiest raw signals in this file (hand-near-face, eyes-closed,
 * head-turn); NOT used for face presence/count, which already has its
 * own grace-period debouncing further down.
 */
function majorityVote(history: boolean[], raw: boolean, windowSize: number): boolean {
  history.push(raw)
  while (history.length > windowSize) history.shift()
  const trueCount = history.reduce((count, value) => count + (value ? 1 : 0), 0)
  return trueCount * 2 > history.length
}

/** A single closed/open eye reading, timestamped for the PERCLOS window below. */
type TimedBoolSample = { t: number; value: boolean }

/**
 * PERCLOS — "percentage of eyelid closure" — the standard drowsiness-
 * research metric: what fraction of the last `windowMs` were the eyes
 * closed, rather than only whether they're closed *right now*. This
 * catches the classic drowsy pattern of heavy, frequent blinking or
 * fluttering eyelids that never holds continuously shut long enough to
 * trip a simple sustained-closure timer on its own, but adds up to
 * "eyes were shut half the time" over a rolling window. Mutates
 * `window` in place, pruning samples older than the window.
 */
function computePerclos(window: TimedBoolSample[], now: number, windowMs: number): number {
  while (window.length > 0 && now - window[0].t > windowMs) window.shift()
  if (window.length === 0) return 0
  const closed = window.reduce((count, sample) => count + (sample.value ? 1 : 0), 0)
...
function bucketFor(state: CameraState): keyof Buckets | null {
  switch (state) {
    case 'FOCUSED':
    // Reading/writing (a sustained, calibrated head-down tilt) is real
    // focused study time, not distraction — it just doesn't look like
    // "facing the screen dead-on". Same bucket, no wire-format change.
    case 'READING':
      return 'focused'
    case 'DISTRACTED':
      return 'distracted'
...
export function useFocusTracker(params: {
  ...
  /**
   * Real phone-object detection (EfficientDet-Lite0, COCO-trained,
   * restricted to the "cell phone" class) — same optional-detector
   * contract as the other refs here. classify() falls back to the
   * coarser hand-near-face proxy if this fails to load.
   */
  const phoneDetectorRef = useRef<ObjectDetector | null>(null)
  ...
  const multipleFacesSinceRef = useRef<number | null>(null)
  const offCenterSinceRef = useRef<number | null>(null)
  const eyesClosedSinceRef = useRef<number | null>(null)
  /** Fallback-path only (no object-detector model): sustained hand-near-face timer. */
  const phoneSinceRef = useRef<number | null>(null)
  /** Real phone detection: accumulates hits from the hand-crop scan into an on/off PHONE signal (see phoneDetection.ts). */
  const phoneEvidenceRef = useRef<PhoneEvidence>(
      createPhoneEvidence({
        windowMs: focusConfig.PHONE_EVIDENCE_WINDOW_MS,
        minHits: focusConfig.PHONE_MIN_HITS,
        strongScore: focusConfig.PHONE_STRONG_CONFIDENCE,
        releaseMs: focusConfig.PHONE_RELEASE_MS,
      }),
  )
  /** Scratch canvas the hand crop is drawn onto for the phone detector — reused every tick. */
  const phoneCropCanvasRef = useRef<HTMLCanvasElement | null>(null)
  /** Sustained mouth-open (jawOpen blendshape) hold — yawn detection, an independent drowsy signal. */
  const yawnSinceRef = useRef<number | null>(null)
  /** Rolling per-tick history buffers feeding majorityVote() for the three noisiest raw signals. */
  const eyesClosedHistoryRef = useRef<boolean[]>([])
  const phoneHistoryRef = useRef<boolean[]>([])
  const headTurnedHistoryRef = useRef<boolean[]>([])
  /** Timestamped eyes-closed samples feeding the PERCLOS rolling window (see computePerclos above). */
  const eyesClosedWindowRef = useRef<TimedBoolSample[]>([])

  // ---- reading/writing (head-down) heuristic ----
  /** Session-local head-pitch calibration samples, same pattern as the posture baseline below. */
  const readingBaselineSamplesRef = useRef<number[]>([])
  const readingBaselineRef = useRef<number | null>(null)
  const readingHistoryRef = useRef<boolean[]>([])
  ...
  const classify = useCallback(() => {
    ...
    // ---- phone in hand (real object detection on a crop around each hand) ----
    // Deliberately evaluated BEFORE (and independent of) face detection:
    // holding a phone low or flat means looking down at it, which is
    // exactly when the face detector loses the face (NO_FACE) or the
    // landmarker reads the lowered eyelids as "eyes closed" (DROWSY).
    // Either of those used to win and the phone was never even checked.
    // Skipped while paused — nothing is counted then, so don't burn CPU.
    let phoneActive = false
    const phoneDetector = phoneDetectorRef.current
    if (phoneDetector && monitoringRef.current) {
      const canvas = (phoneCropCanvasRef.current ??= document.createElement('canvas'))
      const bestScore = detectPhoneInHands(
          phoneDetector,
          video,
          canvas,
          ...
          frameH,
          {
            minScore: focusConfig.PHONE_OBJECT_CONFIDENCE,
            cropScale: focusConfig.PHONE_CROP_SCALE,
            minSideFraction: focusConfig.PHONE_CROP_MIN_SIDE_FRACTION,
            inputSize: focusConfig.PHONE_CROP_INPUT_SIZE,
            maxHands: focusConfig.PHONE_CROP_MAX_HANDS,
            handProximityPadding: focusConfig.PHONE_HAND_PROXIMITY_FRACTION,
          },
      )
      phoneActive = phoneEvidenceRef.current.update(now, bestScore)
      if (focusConfig.PHONE_DEBUG_LOG && (handResult?.landmarks?.length ?? 0) > 0) {
        console.debug('[focus] phone score', bestScore?.toFixed(2) ?? 'none', phoneActive ? '-> PHONE' : '')
      }
    }
    ...
    // Only detections that clear the confidence bar count as a real face.
    // The old code let ANY extra detection — however faint — count toward
    // "how many people", so a low-confidence blob (very often a hand or a
    // phone raised near the face) was enough to flip the state straight
    // to MULTIPLE_FACES, pre-empting the phone/drowsy checks below entirely.
    const confidentDetections = detections.filter(
        (d) => (d.categories?.[0]?.score ?? 0) >= focusConfig.MIN_DETECTION_CONFIDENCE,
    )

    if (confidentDetections.length === 0) {
      if (phoneActive) {
        // Face lost because the head is down over a phone — that's PHONE, not NO_FACE.
    ...
    if (confidentDetections.length > 1) {
      // A second confident face has to be SUSTAINED before this counts as
      // MULTIPLE_FACES — same grace-period treatment every other state
      // here gets. A single flickery extra detection (raising a hand or
      // phone near the face can momentarily read as a second face to the
      // lightweight BlazeFace model) must not instantly override an
      // otherwise correct PHONE/DROWSY/FOCUSED read for that tick.
      if (multipleFacesSinceRef.current === null) multipleFacesSinceRef.current = now
      if (now - multipleFacesSinceRef.current >= focusConfig.MULTIPLE_FACES_GRACE_MS) {
        offCenterSinceRef.current = null
        eyesClosedSinceRef.current = null
        phoneSinceRef.current = null
      ...
      // Not sustained yet — fall through and classify off the strongest
      // single detection below, same as a lone face would be, so a
      // momentary flicker doesn't blank out an otherwise-good read.
    } else {
      multipleFacesSinceRef.current = null
    }

    // A verified phone-in-hand outranks the drowsy/off-center checks
    // below: looking down at a phone lowers the eyelids enough that the
    // eyes-closed blendshape fires, which would otherwise read as DROWSY.
    // (Only with the real detector — the hand-near-face fallback further
    // down keeps its old "drowsy wins" ordering, since a hand at the face
    // there is just as likely to be rubbing tired eyes.)
    if (phoneActive) {
      offCenterSinceRef.current = null
      eyesClosedSinceRef.current = null
    ...
    // Most confident detection wins when classifying a single person —
    // relevant both for the ordinary one-face case and for the "not
    // sustained yet" fall-through above.
    const face = confidentDetections.reduce((best, candidate) =>
        (candidate.categories?.[0]?.score ?? 0) > (best.categories?.[0]?.score ?? 0) ? candidate : best,
    )
    ...
    // ---- drowsy heuristic: multi-signal (sustained closure, PERCLOS, yawn) ----
    // Checked BEFORE the phone heuristic below, and always wins while any
    // of its three signals fire. Rubbing tired eyes or resting your head
    // on a hand is a completely normal thing to do right as your eyes
    // close — if the phone check ran first a hand near the face would
    // wipe eyesClosedSinceRef back to null on every single tick, so the
    // DROWSY grace period could never finish accumulating.
    const landmarker = landmarkerRef.current
    let eyesClosedRaw = false
    let yawningRaw = false
    if (landmarker) {
      ...
      if (blendshapes) {
        ...
        eyesClosedRaw = (left + right) / 2 >= focusConfig.EYE_CLOSED_BLENDSHAPE_THRESHOLD
        const jawOpen = blendshapes.find((c) => c.categoryName === 'jawOpen')?.score ?? 0
        yawningRaw = jawOpen >= focusConfig.YAWN_BLENDSHAPE_THRESHOLD
      }
    }

    // PERCLOS wants the true noisy per-tick signal, not a debounced one —
    // it's already a rolling-window average, so smoothing it twice would
    // just blunt real drowsiness. Push every raw reading in before any
    // majority-vote smoothing happens below.
    eyesClosedWindowRef.current.push({ t: now, value: eyesClosedRaw })
    const perclos = computePerclos(eyesClosedWindowRef.current, now, focusConfig.PERCLOS_WINDOW_MS)
    const perclosDrowsy =
        eyesClosedWindowRef.current.length >= focusConfig.PERCLOS_MIN_SAMPLES &&
        perclos >= focusConfig.PERCLOS_THRESHOLD

    // A single low-confidence frame shouldn't reset an otherwise-real
    // continuous closure — majority-vote over the last few ticks feeds
    // the sustained-closure timer instead of the raw per-tick reading.
    const eyesClosed = majorityVote(eyesClosedHistoryRef.current, eyesClosedRaw, focusConfig.SMOOTHING_WINDOW_SAMPLES)
    ...
    // A sustained yawn is, on its own, a strong and standalone
    // drowsiness signal (unlike a single open-mouth frame, which is
    // just as likely to be talking) — it fires immediately once held
    // this long, same as PERCLOS below, rather than waiting on top of
    // its own grace period for a second timer.
    const sustainedYawn = yawnSinceRef.current !== null && now - yawnSinceRef.current >= focusConfig.YAWN_GRACE_MS
    ...
    const sustainedClosure =
        eyesClosedSinceRef.current !== null && now - eyesClosedSinceRef.current >= focusConfig.DROWSY_GRACE_MS

    if (sustainedClosure || perclosDrowsy || sustainedYawn) {
      // Any one of the three signals is enough: a continuous closed-eyes
      // hold, a rolling window that's mostly closed (heavy/frequent
      // blinking that never holds continuously long enough on its own —
      // the classic real-world drowsy pattern), or a held yawn.
      phoneSinceRef.current = null
      offCenterSinceRef.current = null
      transition('DROWSY')
      return
    ...
    if (eyesClosed) {
      // Eyes currently read closed (majority vote) but not yet sustained
      // long enough to fire on its own — still pre-empt the phone
      // heuristic entirely: don't let a hand that happens to be near the
      // face (mid-blink-rub, propping your chin) start or continue
      // counting toward PHONE while we're on the verge of DROWSY.
      phoneSinceRef.current = null
      offCenterSinceRef.current = null
    } else {
      // ---- phone detection, fallback path only ----
      // With the object-detector model loaded, phone-in-hand was already
      // decided above (phoneActive) and never reaches this point. This
      // block only runs when that model failed to load, using the coarser
      // hand-near-face proxy.
      let phoneRaw = false
      if (!phoneDetectorRef.current) {
        // Fallback for when the object-detector model failed to load:
        // the coarser hand-near-face proxy this hook used before real
        // phone detection was available — same optional-detector
        // degrade-gracefully contract as the other detectors here.
      ...
      // Majority-vote smoothed: a single noisy frame — a missed
      // detection, a hand passing quickly through frame — no longer
      // starts or breaks the PHONE timer on its own.
      const phoneDetected = majorityVote(phoneHistoryRef.current, phoneRaw, focusConfig.SMOOTHING_WINDOW_SAMPLES)

      if (phoneDetected) {
        offCenterSinceRef.current = null
        if (phoneSinceRef.current === null) phoneSinceRef.current = now
        if (now - phoneSinceRef.current >= focusConfig.PHONE_GRACE_MS) {
          transition('PHONE')
          return
        }
      } else {
    ...
    // Head-turn (yaw) + head-down (pitch) heuristics from the detector's
    // facial keypoints (0: right eye, 1: left eye, 2: nose tip). Facing
    // the camera, the nose tip sits near the midpoint between the eyes,
    // a steady distance below the eye line; turning the head sideways
    // pushes it left/right, tipping the head down pushes it further
    // below the eye line. Both normalized by the eye-to-eye distance so
    // they're independent of how close the user sits. Still a simple
    // presence heuristic — NOT gaze tracking.
    let headTurnedAwayRaw = false
    let noseDropRatio: number | null = null
    ...
    if (keypoints && keypoints.length >= 3) {
      ...
      if (interEye > 0.001) {
        ...
        noseDropRatio = (noseTip.y - eyeMidY) / interEye
      }
    }
    // Yaw estimation from two-point keypoints is the noisiest signal
    // here (small keypoint jitter near the threshold flips it easily) —
    // majority-vote smoothed so one bad frame can't cost a FOCUSED tick.
    const headTurnedAway = majorityVote(headTurnedHistoryRef.current, headTurnedAwayRaw, focusConfig.SMOOTHING_WINDOW_SAMPLES)

    // ---- reading/writing heuristic (sustained head-down pitch) ----
    // Looking down at a notebook or textbook is real, focused study
    // time — not distraction — but it moves the face down and out of
    // the central region, which would otherwise misread as DISTRACTED.
    // Session-local calibration, same idea as the posture baseline
    // above: the first few confident readings establish "what looking
    // at the screen looks like" for this person/seat/camera (assumes
    // the session starts with them actually looking at it — same
    // caveat as the posture baseline), and a sustained further drop
    // below that baseline reads as looking down at the desk instead.
    let readingRaw = false
    if (noseDropRatio !== null) {
      if (readingBaselineRef.current === null) {
        readingBaselineSamplesRef.current.push(noseDropRatio)
        if (readingBaselineSamplesRef.current.length >= focusConfig.READING_BASELINE_SAMPLES) {
          const samples = readingBaselineSamplesRef.current
          readingBaselineRef.current = samples.reduce((sum, value) => sum + value, 0) / samples.length
        }
      } else {
        readingRaw = noseDropRatio >= readingBaselineRef.current * focusConfig.READING_PITCH_RATIO_THRESHOLD
      }
    }
    const reading = majorityVote(readingHistoryRef.current, readingRaw, focusConfig.SMOOTHING_WINDOW_SAMPLES)
    ...
    // Not screen-facing FOCUSED, but a sustained head-down tilt with the
    // face otherwise present and facing forward (not turned sideways):
    // reading or writing, not distraction.
    if (bigEnough && !headTurnedAway && reading) {
      offCenterSinceRef.current = null
      if (readingSinceRef.current === null) readingSinceRef.current = now
      if (now - readingSinceRef.current >= focusConfig.READING_GRACE_MS) {
        transition('READING')
        return
      }
    } else {
  ...
  const loadOptionalDetectors = useCallback(async (fileset: Awaited<ReturnType<typeof FilesetResolver.forVisionTasks>>) => {
    // Loaded in parallel (they used to load one after another, which put
    // the phone detector — third in line — a good few seconds behind
    // camera-ready). Each is independently try/caught, so any one failing
    // just leaves its own ref null.
    await Promise.allSettled([
      (async () => {
        try {
          landmarkerRef.current = await createWithGpuFallback(
              (delegate) =>
                  FaceLandmarker.createFromOptions(fileset, {
                    baseOptions: { modelAssetPath: focusConfig.FACE_LANDMARKER_MODEL_URL, delegate },
                    runningMode: 'VIDEO',
                    numFaces: 1,
                    outputFaceBlendshapes: true,
                  }),
              (task, blank) => void task.detectForVideo(blank, performance.now()),
          )
        } catch {
      ...
      (async () => {
        try {
          handDetectorRef.current = await createWithGpuFallback(
              (delegate) =>
                  HandLandmarker.createFromOptions(fileset, {
                    baseOptions: { modelAssetPath: focusConfig.HAND_LANDMARKER_MODEL_URL, delegate },
                    runningMode: 'VIDEO',
                    numHands: 2,
                  }),
              (task, blank) => void task.detectForVideo(blank, performance.now()),
          ...
          // Gesture shortcuts and the phone-detection hand crop
          // simply won't fire this session — same null-check fallback in
          // classify()/detectGestures().
        }
      })(),
      (async () => {
        try {
          // IMAGE mode, not VIDEO: the detector is fed crops from
          // wherever the hands are, not a continuous frame sequence, so
          // there are no timestamps to keep monotonic (see phoneDetection.ts).
          phoneDetectorRef.current = await createWithGpuFallback(
              (delegate) =>
                  ObjectDetector.createFromOptions(fileset, {
                    baseOptions: { modelAssetPath: focusConfig.OBJECT_DETECTOR_MODEL_URL, delegate },
                    runningMode: 'IMAGE',
                    scoreThreshold: focusConfig.PHONE_OBJECT_CONFIDENCE,
                    categoryAllowlist: ['cell phone'],
                    maxResults: 3,
                  }),
              (task, blank) => void task.detect(blank),
          )
        } catch {
          // Real phone detection simply won't fire this session — classify()
          // falls back to the coarser hand-near-face proxy when this ref is
          // null, same null-check fallback pattern as every detector here.
        }
      })(),
      (async () => {
        try {
          poseDetectorRef.current = await createWithGpuFallback(
              (delegate) =>
                  PoseLandmarker.createFromOptions(fileset, {
                    baseOptions: { modelAssetPath: focusConfig.POSE_LANDMARKER_MODEL_URL, delegate },
                    runningMode: 'VIDEO',
                    numPoses: 1,
                  }),
              (task, blank) => void task.detectForVideo(blank, performance.now()),
  ...
  const startCamera = useCallback(async () => {
    ...
    if (reusableStream) {
      ...
      try {
        ...
        const stream = await navigator.mediaDevices.getUserMedia({
          video: {
            width: { ideal: focusConfig.CAMERA_IDEAL_WIDTH },
            height: { ideal: focusConfig.CAMERA_IDEAL_HEIGHT },
            facingMode: 'user',
          },
          audio: false,
        })
    ...
    // Self-scheduling loop rather than setInterval: with five models per
    // tick, a slow tick (CPU-only device, busy tab) used to be followed
    // immediately by the next one, back-to-back, starving the main thread
    // and stuttering the video preview. Now the next tick is scheduled
    // only after this one finishes, aiming for a steady SAMPLE_INTERVAL_MS
    // between tick STARTS but never less than SAMPLE_MIN_GAP_MS of idle
    // time. try/catch so one bad frame can't silently end sampling — a
    // setInterval survived a throw, a timeout chain would not.
    const tick = () => {
      ...
      try {
        ...
        console.warn('[focus] sample tick failed', err)
      }
      if (!startedRef.current) return // stopCamera() ran during this tick
      const elapsed = performance.now() - startedAt
      sampleTimerRef.current = window.setTimeout(
          tick,
          Math.max(focusConfig.SAMPLE_MIN_GAP_MS, focusConfig.SAMPLE_INTERVAL_MS - elapsed),
      )
    }
    sampleTimerRef.current = window.setTimeout(tick, focusConfig.SAMPLE_INTERVAL_MS)
  ...
  const stopCamera = useCallback(
      async (finalFlush = true) => {
        ...
        phoneDetectorRef.current?.close()
        ...
        multipleFacesSinceRef.current = null
        yawnSinceRef.current = null
        eyesClosedHistoryRef.current = []
        phoneHistoryRef.current = []
        phoneEvidenceRef.current.reset()
        phoneCropCanvasRef.current = null
        headTurnedHistoryRef.current = []
        eyesClosedWindowRef.current = []
        // Reset gesture + posture + reading-pitch state so a new session
        // (a fresh startCamera() call, not a pause/resume) recalibrates
        // both baselines from scratch rather than carrying over stale
        // ones from a different sitting position, and gesture/reading
        ...
        readingBaselineSamplesRef.current = []
        readingBaselineRef.current = null
