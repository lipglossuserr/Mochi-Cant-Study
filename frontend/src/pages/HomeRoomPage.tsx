...
import { useCharacter, useCursorAwareness, playHappyMeow } from '@/features/character'
 ...
 *
 * Sprint: Feed/Decor's tray now renders beside the room scene (next
 * to Mochi herself) instead of down by the action buttons — see the
 * flex row wrapping `RoomScene` + the tray `motion.div` below. This is
 * a pure layout change: the tray components, their props, and every
 * drag/drop callback are untouched.
 */
function HomeRoomPage() {
  ...
  // Which SKIN itemKeys currently have an equip PATCH in flight — lets
  // InventoryTray disable/grey a coat's tile for the same request it's
  // already mid-flight for, same idea as useRoomFurniture's placingIds.
  const [equippingSkinKeys, setEquippingSkinKeys] = useState<Set<string>>(new Set())
  useEffect(() => () => {
  ...
  const handleConsumeFood = useCallback(
      async (inventoryEntryId: number) => {
        ...
        if (ok) {
          playDropReaction()
          playHappyMeow() // happy "mrow!" right alongside the drop's squish + heart burst
          await refreshPet()
        }
      },
  ...
  /**
   * A coat/color tile dropped on Mochi (Shop v1.1): equip it via
   * PATCH /api/pet/skin (PetContext.equipPetSkin), which updates
   * pet.equippedSkin and — through Character.tsx's `skin` prop —
   * makes RiveCharacterRenderer swap her one-shot color timeline live.
   * Deliberately NOT routed through `placeFurniture`/`placeItem`: the
   * backend's RoomLayoutService rejects SKIN-category items outright
   * ("Items of category SKIN cannot be placed in the room"), so that
   * path can never change her color — see InventoryTray's doc comment.
   */
  const handleEquipSkin = useCallback(
      async (itemKey: string) => {
        setEquippingSkinKeys((current) => new Set(current).add(itemKey))
        setPlacementWarning(null)
        try {
          await equipPetSkin(itemKey)
          playDropReaction() // same instant, tactile "it landed" feedback a furniture/food drop already gets
        } catch (err) {
          setPlacementWarning(friendlyMessage(err, "Couldn't equip that coat — try again."))
        } finally {
          setEquippingSkinKeys((current) => {
            const next = new Set(current)
            next.delete(itemKey)
            return next
          })
      ...
      [equipPetSkin, playDropReaction],
  )

  const isEquippingSkin = useCallback(
      (itemKey: string) => equippingSkinKeys.has(itemKey),
      [equippingSkinKeys],
  )
  ...
  return (
      ...
      // narrow column reads as a form/card. The extra lg:max-w-* room is
      // reserved for the Feed/Decor tray sitting beside Mochi (see below)
      // — it only ever applies once a tray is actually open, so the room
      // doesn't shift width every time you glance at the page.
      <div
          className={`mx-auto w-full transition-[max-width] duration-300 ease-out ${
              activeTray ? 'max-w-4xl lg:max-w-6xl' : 'max-w-4xl'
          }`}
      >
        <AnimatePresence>
        ...
        <FadeInSection>
          <div ref={roomRef}>
            ...
            {/* Room + tray live side by side once a tray is open, so the
              Feed/Decor items sit right next to Mochi instead of all the
              way down by the action buttons — a much shorter (and much
              easier) drag onto her. Below `lg` there's no room to spare
              beside the room scene, so the tray stacks directly under it
              instead; it's still right below Mochi, never down by the
              buttons. `items-start` keeps the tray pinned to the room's
              top edge — roughly where Mochi's own drop zone sits — on
              both layouts. */}
            <div className="flex flex-col items-stretch gap-5 lg:flex-row lg:items-start lg:gap-6">
              ...
              {/* Feed/Decor tray — rendered right beside (or, on narrow
                screens, right under) Mochi. It's the exact same
                FoodInventoryTray/InventoryTray as before with the exact
                same props; only *where* it sits in the layout changed,
                so every drag/drop, placement, and consume behavior is
                untouched. */}
              <AnimatePresence>
                {activeTray && (
                    <motion.div
                        ...
                        className="w-full shrink-0 rounded-3xl border border-white/50 bg-white/45 p-4 shadow-[0_16px_40px_-18px_rgba(224,112,158,0.35)] backdrop-blur-xl lg:w-64"
                    >
                      {activeTray === 'food' ? (
                          <FoodInventoryTray
                              entries={food}
                          ...
                          <InventoryTray
                              ...
                              isEquippingSkin={isEquippingSkin}
                              equippedSkinKey={pet.equippedSkin}
                          />
                      )}
                    </motion.div>
                )}
        ...
        {/* Room objects — Feed/Decor's tray now renders up beside Mochi
          (see above), so this row is just the four buttons themselves. */}
