...
import SpotifyStudyPlaylist from '@/components/study/SpotifyStudyPlaylist'
...
function StudyRoomPage() {
  ...
  useEffect(() => {
    ...
    const wasUnfocused =
        ...
        previous === 'DROWSY' ||
        previous === 'PHONE'
    // READING counts as focused study (see useFocusTracker's bucketFor)
    // even though it isn't the literal 'FOCUSED' state — recovering
    // into either one after being unfocused should trigger the same
    // glance-back event.
    const isNowFocused = cameraState === 'FOCUSED' || cameraState === 'READING'
    if (wasUnfocused && isNowFocused) {
