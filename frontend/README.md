...
docker build \
  ...
  -t mochi-frontend .
docker run -p 8081:80 mochi-frontend
```
`Dockerfile` builds the static bundle and serves it via nginx (`nginx.conf` has the SPA fallback React Router needs). `VITE_*` args are required at build time, not run time — see the note above.
